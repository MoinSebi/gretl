...
pub fn pan_genome(
    ...
    let private_only: Vec<(String, usize, usize)> = paths.par_chunks(chunk_size).flat_map(| chunk | {
    ...
    let similarity_level: Vec<(usize, usize, usize)> = stats.par_chunks(chunk_size).enumerate().flat_map(| (ii, chunk) | {
        let mut local: Vec<(usize, usize, usize)> = (0..*max_value as usize + 1).map(|x| (0, 0, x)).collect();
